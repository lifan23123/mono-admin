...
const fetchWhitelist = async () => {
  loading.value = true;
  try {
    const res = await getConfigDetail({ ident: "whiteIp" });
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm(`确认要将 IP ${row.ip} 从白名单中移除吗？`, "提示", {
...
<template>
  <div class="backend-whitelist-container p-4">
